...
/** POST /api/events/[eventId]/invites — Create invites */
export const POST: RequestHandler = async ({ params, request, cookies, platform }) => {
	...
	try {
		const data = await apiRequest(ctx, `/v1/events/${params.eventId}/invites`, {
			method: 'POST',
			body
		});
...
/** GET /api/events/[eventId]/invites — List pending invites */
