...
/** GET /api/connections/keys — Get connection key metadata */
export const GET: RequestHandler = async ({ cookies, platform }) => {
	...
	try {
		const data = await apiRequest(ctx, '/v1/connections/keys');
		return json(data);
	} catch (e) {
...
/** POST /api/connections/keys — Save public key + encrypted private key */
